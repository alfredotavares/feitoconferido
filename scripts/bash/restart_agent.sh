#!/bin/bash
# Script para reiniciar agente que não responde - VERSÃO VISUAL 🚀
...
# Função para imprimir com cores e animação
print_colored() {
    local color=$1
    local text=$2
...
# Função para barra de progresso CORRIGIDA (sem bc)
progress_bar() {
    local duration=$1
    local steps=20
    # Usar aritmética do bash ao invés de bc
    local step_duration_ms=$((duration * 1000 / steps))
    
    echo -n "["
    for ((i=0; i<$steps; i++)); do
        echo -n "█"
        # Converter milissegundos para segundos com decimal
...
# NOVO: Função para verificar e instalar dependências
check_and_install_deps() {
    ...
    # Instalar dependências do projeto se existir requirements.txt ou pyproject.toml
...
# NOVO: Verificar dependências primeiro
check_and_install_deps
